...
const char* Vehicle::_flightDistanceFactName =      "flightDistance";         //???о???
...
const char* Vehicle::_flowratemeterFactGroupName =      "FlowRateMeter";                //流速计
...
Vehicle::Vehicle(LinkInterface*             link,
    ...
    , _gpsFactGroup(this)
    , _flowratemeterFactGroup(this)       //自己添加
    ...
    , _estimatorStatusFactGroup(this)
{
    ...
    // PreArm Error self-destruct timer 报警警告自毁定时器
    connect(&_prearmErrorTimer, &QTimer::timeout, this, &Vehicle::_prearmErrorTimeout);
    _prearmErrorTimer.setInterval(_prearmErrorTimeoutMSecs);
    _prearmErrorTimer.setSingleShot(true);

    // Send MAV_CMD ack timer 发送mav确认定时器
...
//    mytimer.start();
    ...
    if (_highLatencyLink || link->isPX4Flow()) {
        ...
        _setMaxProtoVersion(100);  //设置最大协议版本
        _setCapabilities(0);       //设置功能
...
void Vehicle::_commonInit()
{
    ...
    _addFactGroup(&_flowratemeterFactGroup,          _flowratemeterFactGroupName);    //自己添加
...
void Vehicle::_handleServo(mavlink_message_t& message){
...
void Vehicle::_handleStatusText(mavlink_message_t& message, bool longVersion)
{
    ...
    QString StopSpraying = "Stop Spraying";
    if(messageText == Liquidtext ){
        Data::liquidlevel  = 0;
...
void Vehicle::_handleVfrHud(mavlink_message_t& message)
{
    ...
    //自己添加获取地速
    Data::groundspeedvalue = double(qIsNaN(vfrHud.groundspeed) ? 0 : vfrHud.groundspeed);
}
...
void Vehicle::_handleAttitudeTarget(mavlink_message_t& message)
{
    ...
    Data::roll = double (qRadiansToDegrees(roll));
    Data::pitch = double (qRadiansToDegrees(pitch));
    Data::yaw = double (qRadiansToDegrees(yaw));
    Data::rollRate = double (qRadiansToDegrees(attitudeTarget.body_roll_rate));
    Data::pitchRate = double (qRadiansToDegrees(attitudeTarget.body_pitch_rate));
    Data::yawRate = double (qRadiansToDegrees(attitudeTarget.body_yaw_rate));
...
//    QString timestrr=datetime.currentDateTime().toString("yyyy-MM-dd HH:mm:ss");
//    qDebug()<<"receive : "<<timestrr<<(gpsRawInt.lat* 1e-7)<<(gpsRawInt.lon* 1e-7) ;
//    float timestrr = timestr.toFloat();
//    Data::time = timestrr.toDouble();
//    Data::time = timestrr;
//    if(Data::update_starttime == false){
//       Data::starttime = Data::time;
//       Data::update_starttime = true;
//    }
    //自己添加 获取实时位置
    Data::lat = double (gpsRawInt.lat* 1e-7);
    Data::lon = double (gpsRawInt.lon* 1e-7);

//    Data::alt = double (gpsRawInt.alt/ 1000.0);
}
...
void Vehicle::_handleGlobalPositionInt(mavlink_message_t& message)
{
    ...
    //自己添加，获取相对高度（气压计高度？）
    Data::alt = double (globalPositionInt.relative_alt / 1000.0);
...
{
//    //数据库连接判断
//    mySqlhelper *task = new mySqlhelper;
    ...
    if (_armed != armed) {
        ...
        if (_armed) {
            ...
            //自己添加，启动记录作业面积，
            Data::save_plan_num=0;
            Data::workingarea=0;
            mytimer.start();//启动定时器，数据开始保存

            //自己添加获取起飞时间，每上锁一次重新获取一次
            QDateTime datetime;
            qint64 timestrr=datetime.currentDateTime().toTime_t();
            qDebug()<<"Just : "<<timestrr ;
            if(Data::update_starttime == false){
...
//            //自己添加，飞机降落，停止定时器
            mytimer.stop();            
            Data::update_starttime = false;

            qDebug()<< QString::fromUtf8("Thread close test:");
...
//    自己添加更新飞行时间
//    Data::flighttime = (double)_flightTimer.elapsed() / 1000.0;
}
...
//    qDebug() << " Current flight mode"  << flightMode << Data::flightmode;
...
//    spraystate()->setRawValue(Data::spraystate);
//    flowrate()->setRawValue(std::numeric_limits<float>::quiet_NaN());
}
...
void Vehicle::updateFlightDistance(double distance)
{
    ...
    if(Data::spraystate > 0){
    //自己添加更新获取飞行距离并计算作业面积
    Data::workingarea = Data::workingarea + ((distance * 8) * 0.0015);  //8m喷幅，单位亩
    }
}
