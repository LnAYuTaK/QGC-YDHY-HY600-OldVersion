...
//自己添加flow rate meter
#define MAVLINK_MSG_ID_FLOW_RATE_RAW_INT 48

MAVPACKED(
typedef struct __mavlink_flowratemeter_raw_int_t {

 uint8_t  spraystate;
 double  flowrate;

}) mavlink_flowratemeter_raw_int_t;
 ...
 * @param gps_raw_int C-struct to decode the message contents into
 */
static inline void mavlink_msg_flowratemeter_raw_int_decode(const mavlink_message_t* msg, mavlink_flowratemeter_raw_int_t* flowratemeter_raw_int)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS

    flowratemeter_raw_int->spraystate = mavlink_msg_flowratemeter_raw_int_get_spraystate(msg);
    flowratemeter_raw_int->flowrate = mavlink_msg_flowratemeter_raw_int_get_flowrate(msg);

#endif
}

static inline uint8_t mavlink_msg_flowratemeter_raw_int_get_spraystate(const mavlink_message_t* msg)
{
    return _MAV_RETURN_uint8_t(msg,  0);
}

static inline double mavlink_msg_flowratemeter_raw_int_get_flowrate(const mavlink_message_t* msg)
{
