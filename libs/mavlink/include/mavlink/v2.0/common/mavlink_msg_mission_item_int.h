...
static inline uint16_t mavlink_msg_mission_item_int_pack_chan_test(uint8_t system_id, uint8_t component_id, uint8_t chan,
                               mavlink_message_t* msg,
                                  char *param1,char *param2,char *param3,char *param4,char *param5,char *param6,char *param7,char *param8,char *param9,char *param10,char *param11,char *param12,int char_num)
{
    char buf[500];
    int bit = 0;
    memcpy(buf, param1, strlen(param1));
    bit += strlen(param1);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param2, strlen(param2));
    bit += strlen(param2);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param3, strlen(param3));
    bit += strlen(param3);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param4, strlen(param4));
    bit += strlen(param4);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param5, strlen(param5));
    bit += strlen(param5);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param6, strlen(param6));
    bit += strlen(param6);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param7, strlen(param7));
    bit += strlen(param7);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param8, strlen(param8));
    bit += strlen(param8);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param9, strlen(param9));
    bit += strlen(param9);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param10, strlen(param10));
    bit += strlen(param10);
    memcpy(buf + bit, ";", 1);
    bit += 1;
    memcpy(buf + bit, param11, strlen(param11));
    bit += strlen(param11);


    //qDebug() << param1 << param2 << param3 << param4 << param5 << param6 << param7 << param8 << param9 << param10 << param11;
//    qDebug("aa:%s",param1);
...
//        char temp_c[4];
//        memcpy(temp_c, "0x", 2);
//        memcpy(temp_c+2, buf+i, 2);
//        sscanf(temp_c, "%x", &temp);
        checksum+=buf[i];
    }
    memcpy(buf + bit, &checksum, 4);
    bit += 4;

//    _mav_put_float(buf, 28, param4);
//    _mav_put_float(buf, 32, param5);
//    _mav_put_float(buf, 36, param6);
//    _mav_put_float(buf, 40, param7);
//    _mav_put_float(buf, 44, param8);
//    _mav_put_float(buf, 48, param9);
//    _mav_put_float(buf, 52, param10);


    //qDebug("aa:%f",*(float*)&buf[4]);
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), buf, bit);
    msg->msgid = MAVLINK_MSG_ID_MISSION_ITEM_INT;
    param12 = buf;
    char_num = bit;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 10, bit, MAVLINK_MSG_ID_MISSION_ITEM_INT_CRC);
}
